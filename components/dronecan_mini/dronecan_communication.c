...
void dronecan_init(CanardOnTransferReception on_reception,
                   CanardShouldAcceptTransfer should_accept)
{
  ...
  canardInit(&g_canard, g_canard_memory_pool, sizeof(g_canard_memory_pool),
                        ...
                        uint8_t *transfer_id)
{
  ...
  int16_t result = canardBroadcast(&g_canard, signature, type_id, transfer_id,
                                   priority, payload, len);
...
bool dronecan_respond(uint8_t destination_node_id, uint8_t *inout_transfer_id,
                      ...
                      const void *payload, uint16_t len)
{
  int16_t result = canardRequestOrRespond(
      &g_canard, destination_node_id, signature, type_id, inout_transfer_id,
      priority, CanardResponse, payload, len);
                      ...
                      const void *payload, uint16_t len)
{
  int16_t result = canardRequestOrRespond(
      &g_canard, destination_node_id, signature, type_id, inout_transfer_id,
      priority, CanardRequest, payload, len);
...
CanardInstance *get_dronecan_instance() { return &g_canard; }

uint64_t get_logical_error() { return logical_error; }

uint64_t get_logical_tx() { return logical_tx; }

void increase_logical_error() { logical_error++; }

void set_node_id(uint8_t node_id) { canardSetLocalNodeID(&g_canard, node_id); }