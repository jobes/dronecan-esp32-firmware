...
get_extra_iface_stats(uavcan_protocol_GetTransportStats_CanIfaceStats *stats)
{
  return 0;
}
...
{
  logical_rx++;
  if (dronecan_extra_on_transfer_received(ins, transfer))
  {
    return;
  }
  ...
  else if (transfer->transfer_type == CanardTransferTypeRequest)
  {
    ...
    case UAVCAN_RESTART_NODE_ID:
    {
      ...
      {
        response_5_restartNode(transfer->source_node_id,
                               &transfer->transfer_id);

        esp_timer_handle_t timer;
        esp_timer_create(&(esp_timer_create_args_t){.callback = restart},
                         &timer);
    ...
    {
      uint16_t param_index = decode_11_paramGetSet_request(
          transfer, get_device_parameters(), get_device_parameters_len());

      if (param_index < get_device_parameters_len())
      {
        response_11_paramGetSet(transfer->source_node_id,
                                &transfer->transfer_id,
                                &get_device_parameters()[param_index]);
      }
      else
      {
        response_11_paramGetSetEmpty(transfer->source_node_id,
                                     &transfer->transfer_id);
      }
      break;
    }
    case UAVCAN_PROTOCOL_PARAM_EXECUTE_OPCODE_ID:
      response_10_paramExecuteOpcode_process(
          transfer, process_parameters_to_nvs(
                        decode_10_paramExecuteOpcode_process(transfer)));
      break;
    case UAVCAN_FILE_BEGIN_FIRMWARE_UPDATE_ID:
    {
      if (*get_node_mode() == MODE_SOFTWARE_UPDATE)
      {
        response_40_beginFirmwareUpdate(IN_PROGRESS, transfer->source_node_id,
                                        &transfer->transfer_id);
        break;
      }
      ...
      char *firmware_path;
      process_40_beginFirmwareUpdate(transfer, &firmware_source_node_id,
                                     &firmware_path);
      if (beginFirmwareUpdate(firmware_source_node_id, firmware_path))
      {
        response_40_beginFirmwareUpdate(OK, transfer->source_node_id,
                                        &transfer->transfer_id);
        request_read_48(get_firmware_source_node_id(), 0, get_firmware_path());
      }
      else
      {
        response_40_beginFirmwareUpdate(INVALID_MODE, transfer->source_node_id,
                                        &transfer->transfer_id);
      }
    ...
    case UAVCAN_PROTOCOL_GET_TRANSPORT_STATS_ID:
    {
      ...
      uint64_t phys_err =
          (uint64_t)status.rx_missed_count + (uint64_t)status.rx_overrun_count +
          (uint64_t)status.tx_failed_count + (uint64_t)status.arb_lost_count +
          (uint64_t)status.bus_error_count;

      uavcan_protocol_GetTransportStats_CanIfaceStats ifaces[3];
      ifaces[0].tx_requests = get_physical_tx();
      ifaces[0].rx_messages = get_physical_rx();
      ifaces[0].errors = phys_err;

      uint8_t num_ifaces = 1 + get_extra_iface_stats(&ifaces[1]);

      response_4_getTransportStats(
          transfer->source_node_id, &transfer->transfer_id, get_logical_error(),
          get_logical_tx(), logical_rx, ifaces, num_ifaces);
      break;
    }
    default:
      ESP_LOGW(TAG, "UNKNOWN TRANSFER RECEIVED: type_id=%d, source_node_id=%d",
