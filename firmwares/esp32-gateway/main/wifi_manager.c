...
static void mdns_start_service(void)
{
  ...
  ESP_ERROR_CHECK(mdns_instance_name_set(DEVICE_INSTANCE_NAME));
  ...
  mdns_service_add(NULL, "_cannelloni", "_udp", CANNELLONI_UDP_PORT, NULL, 0);

  ESP_LOGI(TAG, "mDNS started: hostname='%s.local', instance='%s'", mdns_name,
...
static void reconnect_timer_cb(TimerHandle_t xTimer)
{
  ESP_LOGW(
      TAG,
      "Reconnection timer expired. Retrying STA connection in background...");
  ...
  if (sta_ssid && strlen(sta_ssid) > 0)
  {
    ...
    strncpy((char *)wifi_config.sta.password, sta_pass ? sta_pass : "",
                          ...
                          int32_t event_id, void *event_data)
{
  ...
  {
    // Ensure Power Save is disabled for AP mode too
    esp_wifi_set_ps(WIFI_PS_NONE);
  }
           ...
           event_id == WIFI_EVENT_STA_DISCONNECTED)
  {
    if (s_retry_num < MAXIMUM_RETRY)
    {
      esp_wifi_connect();
      s_retry_num++;
      ESP_LOGW(TAG, "Retry connection to AP (%d/%d)", s_retry_num,
               MAXIMUM_RETRY);
    }
    else
    {
      ...
      else
      {
        ESP_LOGW(
            TAG,
            "STA connection failed after 5 retries. Enabling fallback AP.");
  ...
  else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
  {
    ...
    // Disable Power Save to prevent "sleep" (Modem Sleep) which causes latency
           ...
           event_id == WIFI_EVENT_AP_STACONNECTED)
  {
    ...
    ESP_LOGI(TAG, "Station " MACSTR " joined AP, AID=%d", MAC2STR(event->mac),
             event->aid);
    esp_ota_mark_app_valid_cancel_rollback();
  }
  else if (event_base == WIFI_EVENT &&
           event_id == WIFI_EVENT_AP_STADISCONNECTED)
  {
    ...
    ESP_LOGI(TAG, "Station " MACSTR " left AP, AID=%d", MAC2STR(event->mac),
             event->aid);
  }
  else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED)
  {
    esp_wifi_set_ps(WIFI_PS_NONE);
    ESP_LOGI(TAG, "Connected to AP successfully (PS_NONE set)");
  }
}
...
static void wifi_ap_fallback_start(void)
{
  ...
  wifi_config_t wifi_config = {
      .ap =
          {
              .channel = 6,
              .max_connection = CANNELLONI_MAX_PEERS,
  ...
  // Using string length protection. Mac length is 32 and 64 for SSID and
  // PASSWORD inside wifi_config_t.
  ...
  // Disable Power Save for the fallback AP as well
  esp_wifi_set_ps(WIFI_PS_NONE);

  ESP_LOGI(TAG,
           "Fallback AP Started successfully. SSID: '%s'. Background STA retry "
           "active.",
           ap_ssid);
  ...
  {
    s_reconnect_timer = xTimerCreate("reconn_timer", pdMS_TO_TICKS(10000),
...
{
  ESP_LOGW(TAG, "Starting WiFi in STA mode. Attempting to connect to configured SSID...");
  char *sta_ssid = get_device_parameter("STA_SSID")->String.value;
  char *sta_pass = get_device_parameter("STA_PASSWORD")->String.value;

  ESP_LOGW(TAG, "Configured STA_SSID: '%s'", sta_ssid ? sta_ssid : "<empty>");
  if (!sta_ssid || strlen(sta_ssid) == 0)
  {
    ESP_LOGW(
        TAG,
        "STA_SSID is empty, skipping STA and starting Fallback AP directly.");
    wifi_ap_fallback_start();
    return;
  }
  ...
  ESP_LOGI(TAG, "STA start finished. Trying to connect to SSID: '%s'",
           sta_ssid);
}
...
void wifi_init_manager(void)
{
  ...
  if (hostname_param && hostname_param->String.value &&
      strlen(hostname_param->String.value) > 0)
  {
  ...
  esp_wifi_restore(); // vymaže WiFi NVS storage
